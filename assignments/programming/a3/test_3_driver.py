...
#original file name in coursera a3_driver.py
...
def print_board(board):
    ...
    for row_idx in range(len(board)):
        print(test_3.make_str_from_row(board, row_idx))
...
def play_game(players, board, words):
    ...
    """
    num_remaining = test_3.num_words_on_board(board, words) - len(found_words)
    player_num = 0
    while num_remaining > 0:
        ...
        if test_3.is_valid_word(words, guess) and test_3.board_contains_word(board, guess) and \
            not guess in found_words:
            test_3.update_score(players[player_num % len(players)], guess)
...
words = test_3.read_words(words_file)
words_file.close()

# Load the board.
board_file = askopenfile(mode='r', title='Select board file')
board = test_3.read_board(board_file)
board_file.close()
