...
for f in EXPECTED_FNS:
    if not hasattr(test_1, f):
        print(f, "is missing in test_1.py.")
        test_1.__dict__[f] = lambda *x: 0
...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.diff = test_1.hours_difference(dlt, 0)
    ...
    def refresh(self, forced=False):
        ...
        hour = int(test_1.time_from_utc(utc_hour, self.diff))
        ...
        minutes_angle = test_1.get_minutes(int(self.now)) / 60 * 360
        if test_1.time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif test_1.time_from_utc(0, self.diff) % 1 == 0.75:
            minutes_angle += 270
        ...
        seconds_angle = int(test_1.get_seconds(int(self.now))) / 60 * 360
